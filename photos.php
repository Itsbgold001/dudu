...
if($u == $log_username && $user_ok == true){
	...
	$photo_form .=     '<option value="Myself">Me</option>';
...
<style type="text/css">
form#photo_form{background:#F3FDD0; border:#AFD80E 1px solid; padding:20px; width: auto;}
div#galleries{
	padding: 10px;
	}
div#galleries > div{float:left; margin:10px; text-align:center; cursor:pointer; background:#FFF; border-radius: 8px;
padding: 10px;
}
div#galleries > div > div {height:200px; overflow:hidden;}
div#galleries > div > div > img{width:200px; cursor:pointer; height:200px;}
div#photos{display:none; border:#666 1px solid; padding:20px; height: auto; width: 850px; margin: 10px; background-color: #000; }
div#photos > div{float:left; width:160px; height:80px; overflow:hidden; margin:10px; height: 150px; border-radius: 4px; background-color:#FFF; padding: 10px; border: outset thin; }
div#photos > div > img{width:160px; height: 150px; cursor:pointer;}
div#picbox{display:none; padding-top:36px;}
div#picbox > img{max-width:600px; max-height:500px; display:block; margin:0px auto; border: outset thin; }
div#picbox > button{ background-color: #F30; border-radius: 20px; opacity: 0.5; display: block; margin-right: 100px; float:right; font-size:26px; padding:1px; height: 40px; width: 40px;}
...
<?php  
	 	$thumbquery = mysqli_query($db_conx, "SELECT avatar FROM users WHERE username='$u' LIMIT 1");
		$thumbrow = mysqli_fetch_row($thumbquery);
		$albumavatar = $thumbrow[0];
		$albumpic = '<img src="user/'.$u.'/'.$albumavatar.'" alt="'.$u.'" class="user_pic">';
		?>
...
function showGallery(gallery,user){
	...
	_("section_title").innerHTML = '<a href="user.php?u=<?php echo $u; ?>"><?php echo $u; ?></a><?php echo $albumpic; ?>&#39;s '+gallery+' Gallery &nbsp; <button onclick="backToGalleries()">Go back to all galleries</button>';
...
function photoShowcase(picdata){
	...
	_("picbox").innerHTML += '<div id="pic_comments">###Photo Comments go here###</div>';
...
//Still working on the next photo function below...
function nextPhoto(){
	_("picbox").innerHTML = '<img src="user/<?php echo $u; ?>/'+data[i]+'" alt="photo">';
	_("picbox").style.display = "none";
	_("photos").style.display = "block";
	_("section_title").style.display = "block";
}
function deletePhoto(id){
	var conf = confirm("Are you sureyou want to delete this photo?");
	if(conf != true){
		return false;
	}
...
<div id="gallery">
  ...
  <div id="galleryHead"><h2 id="section_title"><?php echo $albumpic; ?><a href="user.php?u=<?php echo $u; ?>"><?php echo $u; ?></a>&#39;s Photo Galleries</h2></div>
