...
$profile_pic = "";
if($isOwner == "yes"){
	$status_ui = '<div id="statuspost"><textarea id="statustext" rows="4" cols="4" onkeyup="statusMax(this,150)" placeholder="What&#39;s up '.$u.'?"></textarea><button class="uibutton" type="button" id="upload_pic">Upload Picture</button>';
	
	
	$status_ui .= '<button id="statusBtn" onclick="postToStatus(\'status_post\',\'a\',\''.$u.'\',\'statustext\')">Post</button></div>';
} else if($isFriend == true && $log_username != $u){
	$status_ui = '<div id="statuspost"><textarea id="statustext" onkeyup="statusMax(this,250)" placeholder="Hi '.$log_username.', say something to '.$u.'"></textarea>';
	$status_ui .= '<button id="statusBtn" onclick="postToStatus(\'status_post\',\'c\',\''.$u.'\',\'statustext\')">Post</button></div>';
}
?>
...
ORDER BY s.postdate DESC LIMIT 40";
...
$when2 = ($timeAgoObject -> makeAgo($convertedTime)); // Then convert to ago time
	...
	$statuslist .= '<div id="statusuiBox"><div id="status_'.$statusid.'" class="status_boxes" ><div><b><div id="statusAvtr">'.$user_image.'</div><a href="user.php?u='.$author.'">'.$author.'</a> '.$when.':</b> '.$statusDeleteButton.' <br /><div id="postText"><div>'.$data.'</div></div><br />
	<div class="voting_wrapper" id="vote_'.$statusid.'">
