...
if (in_array($_SESSION['username'],$moderators)){
	$addMembers = '<div id="GroupNote"><h3>Group Requests</h3><hr>';
	for($x=0;$x<$pend_count;$x++){		
	
		$thumbquery = mysqli_query($db_conx, "SELECT avatar FROM users WHERE username='$pending[$x]' LIMIT 1");
		$thumbrow = mysqli_fetch_row($thumbquery);
		$groupavatar = $thumbrow[0];
		$grouppic = '<img src="user/'.$pending[$x].'/'.$groupavatar.'" alt="'.$pending[$x].'" class="user_pic">';
		
		$addMembers .= '<div>'.$grouppic.'<a href="user.php?u='.$pending[$x].'">'.$pending[$x].'</a></div>';
		$addMembers .= '<button id="appBtn" onClick="approveMember(\''.$pending[$x].'\')">Approve</button>';
		$addMembers .= '<button id="appBtn" onClick="declineMember(\''.$pending[$x].'\')">Decline</button><br /><hr>';
	}		
}
// Add admin
if (in_array($_SESSION['username'],$moderators)){
	...
	$addAdmin .= '<button id="addAdm" onClick="addAdmin()">Add</button></div>';	
}
...
if($numrows > 0){
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		$mainPosts .= '</div>';
		$mainPosts .= '<div id="postDetails">'.$post_auth.' ---- '.date('F d, Y - g:ia', strtotime($post_date));
		$mainPosts .= '</div><br />';
		$mainPosts .= '<div class="postsWords">';
		$mainPosts .= $post_data;
		$mainPosts .= '</div>';
		$mainPosts .= '<div class="voting_wrapper" id="vote_'.$post_id.'">
        ...
        </div> <div class="rateWrapper">###</div><br />';		
	    ...
	    $numrows2 = mysqli_num_rows($query2);
	 	if($numrows2 > 0){
			while ($row2 = mysqli_fetch_array($query2, MYSQLI_ASSOC)) {
				...
				$mainPosts .= '<hr/>';
				// Build replies
			 	$mainPosts .= '<div class="postsReplies">';
			    $mainPosts .= '<div class="postsPic">';
			  	$mainPosts .= $reply_image;
			  	$mainPosts .= '</div>';
			  	$mainPosts .= '<div class="postsWords">';
				$mainPosts .= $reply_auth.' - '.date('F d, Y - g:ia', strtotime($reply_date)).'<br /><br />';
    	...
    	$mainPosts .= '<br /><button id="reBtn" onClick="replyPost(\''.$post_id.'\')">Post</button>';
...
function quitGroup(){
	var conf = confirm("Are you sure you want to quit the group?");
	if(conf != true){
		return false;
	}
	var ajax = ajaxObj("POST", "php_parsers/group_parser.php");
...
<script src="js/jquery-1.8.1.min.js"></script>
  ...
  <p>Created on: <?php echo date('F d, Y', strtotime($gCreation)); ?></p>
  <p><!--This group has <?php echo $mod_count; ?> moderators, -->Group members: <?php echo $app_count; ?> <!--and <?php echo $pend_count; ?> members pending approval--></p>
