/**
* Search 2.0 - jQuery plugin for dudu type search suggestions
*
* Malcolmmaima
*
* Copyright (c) 2014 dudu
*/
(function($){
$.fn.fsearch = function(){
  ...
  var q=$(this).val();
    ...
    { 
      ...
      // When enter key is pressed
        ...
        $resultDiv.fadeOut('id','.name');// Here you get the id and name of the element selected. You can change this to redirect to any page too. Just like facebook.   
      }
      else{
      $resultDiv.fadeIn();
      $resultDiv.find('#search-footer').html("<img src='images/loading.gif' alt='Searching...'/>");
        ...
        for(var i=0; i<jsonResult.length;i++)
          {
            ...
            str += '<a href="user.php?u=' + jsonResult[i].username + '"><li id=' + jsonResult[i].uid + ' class="option"><img class="profile_image" src="user/'+jsonResult[i].avatar+'" alt="'+jsonResult[i].username+'"/><span class="name">' + jsonResult[i].username + '</span><br/>'+jsonResult[i].country+'</li></a>';
          }
          $resultDiv.find('ul').empty().prepend(str);
		  // Need to run a conditional statement here to LIMIT RESULTS FOR VIEW MORE SEARCH RESULTS
